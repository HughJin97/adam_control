...
class ImprovedSingleFootBalanceSimulation:
    """改进版单脚平衡仿真类"""
    ...
    def __init__(self, model_path: str = "models/scene.xml"):
        ...
        # 改进的控制器参数 - 大幅降低增益
        self.kp_joint = 20.0   # 关节位置增益 (从100降到20)
        self.kd_joint = 5.0    # 关节速度增益 (从10降到5)
        self.kp_balance = 50.0 # 平衡控制增益 (从500降到50)
        self.kd_balance = 10.0 # 平衡阻尼增益 (从50降到10)
        
        # 力矩限制
        self.max_joint_torque = 50.0  # 最大关节力矩限制
    ...
    def _get_single_foot_stance_pose(self) -> Dict[str, float]:
        ...
        if self.support_foot == "right":
            ...
            pose['J_hip_r_pitch'] = -0.05  # 减小前倾角度
            pose['J_knee_r_pitch'] = 0.1   # 减小弯曲角度
            pose['J_ankle_r_pitch'] = -0.05
            pose['J_ankle_r_roll'] = 0.0
            
            # 左腿 - 摆动腿，减小抬起幅度
            pose['J_hip_l_roll'] = 0.0
            pose['J_hip_l_yaw'] = 0.0
            pose['J_hip_l_pitch'] = 0.5    # 减小抬起角度 (从0.8降到0.5)
            pose['J_knee_l_pitch'] = -0.8  # 减小弯曲角度 (从-1.2降到-0.8)
            pose['J_ankle_l_pitch'] = 0.3  # 减小角度
            ...
            pose['J_hip_l_roll'] = 0.0
            pose['J_hip_l_yaw'] = 0.0
            pose['J_hip_l_pitch'] = -0.05
            pose['J_knee_l_pitch'] = 0.1
            pose['J_ankle_l_pitch'] = -0.05
            pose['J_ankle_l_roll'] = 0.0
            
            # 右腿 - 摆动腿
            pose['J_hip_r_roll'] = 0.0
            pose['J_hip_r_yaw'] = 0.0
            pose['J_hip_r_pitch'] = 0.5
            pose['J_knee_r_pitch'] = -0.8
            pose['J_ankle_r_pitch'] = 0.3
            pose['J_ankle_r_roll'] = 0.0
        
        # 手臂 - 使用更保守的平衡姿态
        pose['J_arm_l_01'] = 0.3   # 减小角度
        pose['J_arm_l_02'] = 0.2
        pose['J_arm_l_03'] = -0.3
        pose['J_arm_l_04'] = 0.5   # 减小角度
        pose['J_arm_l_05'] = 0.0
        pose['J_arm_l_06'] = 0.0
        pose['J_arm_l_07'] = 0.0
        
        pose['J_arm_r_01'] = -0.3  # 减小角度
        pose['J_arm_r_02'] = -0.2
        pose['J_arm_r_03'] = 0.3
        pose['J_arm_r_04'] = 0.5   # 减小角度
        pose['J_arm_r_05'] = 0.0
        pose['J_arm_r_06'] = 0.0
        pose['J_arm_r_07'] = 0.0
    ...
    def initialize_robot_pose(self):
        ...
        self.data.qpos[base_pos_idx:base_pos_idx+3] = [0.0, 0.0, 0.9]  # 降低高度到0.9m
    ...
    def compute_balance_control(self, robot_state: Dict) -> Dict:
        ...
        # 限制水平力的大小
        max_horizontal_force = 20.0  # 大幅减小最大水平力
                ...
                if qpos_idx < len(self.data.qpos) and qvel_idx < len(self.data.qvel):
                    ...
                    if self.support_foot == "right" and joint_name in ['J_ankle_r_pitch', 'J_ankle_r_roll']:
                        if joint_name == 'J_ankle_r_pitch':
                            balance_adjustment = balance_control['balance_error'][0] * 0.02  # 大幅减小增益
                        elif joint_name == 'J_ankle_r_roll':
                            balance_adjustment = balance_control['balance_error'][1] * 0.02
                    elif self.support_foot == "left" and joint_name in ['J_ankle_l_pitch', 'J_ankle_l_roll']:
                        if joint_name == 'J_ankle_l_pitch':
                            balance_adjustment = balance_control['balance_error'][0] * 0.02
    ...
    def run_simulation(self, duration: float = 10.0, use_viewer: bool = True):
        ...
        print(f"开始改进版单脚平衡仿真 (支撑脚: {self.support_foot})")
        print(f"仿真时长: {duration}秒")
        print("控制参数已优化，避免跳跃和抽搐")
        print("按 Ctrl+C 停止仿真")
    ...
    def plot_results(self):
        ...
        fig.suptitle(f'改进版单脚平衡仿真结果 (支撑脚: {self.support_foot})')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='改进版单脚平衡仿真')
    ...
    # 创建仿真实例
    sim = ImprovedSingleFootBalanceSimulation()
